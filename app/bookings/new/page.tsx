...
function NewBookingForm() {
  ...
  useEffect(() => {
    ...
    Promise.all(months.map(m => fetch(`/api/bookings?month=${m}`).then(r => r.json()))).then(results => {
      const all = results.flat() as Array<{ status: string; is_pencil?: number; booking_date: string }>;
      setBookedDates(all.filter(b => b.status !== 'cancelled' && !b.is_pencil).map(b => b.booking_date));
    });
