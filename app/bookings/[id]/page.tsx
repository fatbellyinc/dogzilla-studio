        ...
        <select value={selectedId} onChange={e => setSelectedId(Number(e.target.value))}
          ...
          {bookingDays.map((d, i) => (
            <option key={d.id} value={d.id}>
              Day {i + 1} — {shortDayLabel(d.date)} ({d.day_type === 'setup' ? 'Setup' : d.day_type === 'cancelled' ? 'Cancelled' : 'Shoot'})
            </option>
          ))}
        </select>
      </div>

      <div className="text-xs text-white/30 mb-2">{dayLabel} — {day.day_type === 'setup' ? '🔧 Setup Day' : day.day_type === 'cancelled' ? '🚫 Cancelled' : '🎬 Shoot Day'}</div>
...
export default function BookingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function startEditDates() {
    setEditDays((data!.bookingDays || []).map(d => {
      const rate = STUDIO_RATES[d.studio_rate as keyof typeof STUDIO_RATES];
      const base = d.studio_rate === 'hourly' ? rate.price * d.hours : rate.price;
      return {
        date: d.date,
        day_type: d.day_type,
        ...
        cancellation_pct: d.day_type === 'cancelled' && base > 0 ? Math.round((d.subtotal / base) * 100) : 50,
      };
    }));
                  ...
                  {bookingDays.map((d, i) => (
                    <div key={d.id} className="flex justify-between text-xs py-0.5">
                      <span className={d.day_type === 'setup' ? 'text-yellow-400' : d.day_type === 'cancelled' ? 'text-orange-400' : 'text-white/60'}>
                        ...
                        {' '}({d.day_type === 'setup' ? '🔧 Setup' : d.day_type === 'cancelled' ? '🚫 Cancelled' : '🎬 Shoot'})
