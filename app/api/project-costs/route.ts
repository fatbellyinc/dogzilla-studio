...
export async function POST(req: NextRequest) {
  ...
  const { project_id, category, description, note, internal_cost, client_cost, contact_id } = body;
  ...
  const result = db.prepare(`
    INSERT INTO project_costs (project_id, category, description, note, internal_cost, client_cost, sort_order, contact_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(project_id, category, description, note || null, Number(internal_cost) || 0, Number(client_cost) || 0, max + 1, contact_id || null);
