...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const ins = db.prepare('INSERT INTO booking_equipment (booking_id, equipment_id, quantity, rate, name, item_type, is_complimentary, discount_pct, day_date, category, is_tbd) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
  for (const item of equipment_items || []) {
    ins.run(id, item.equipment_id || null, item.quantity, item.rate, item.name, item.item_type || 'individual', item.is_complimentary ? 1 : 0, item.discount_pct || 0, item.day_date || null, item.category || null, item.is_tbd ? 1 : 0);
  }
