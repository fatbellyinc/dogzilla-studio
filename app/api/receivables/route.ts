...
export async function GET() {
  ...
  // All bookings with outstanding balance (not cancelled, not fully paid)
  const bookings = db.prepare(`
    SELECT
      ...
      b.no_deposit, b.vat_exempt, COALESCE(b.fully_paid, 0) as fully_paid,
      CASE WHEN b.vat_exempt = 1 THEN b.total ELSE ROUND(b.total * 1.12, 2) END as total_with_vat,
      c.name as client_name, c.phone as client_phone, c.email as client_email,
      COALESCE(SUM(p.amount), 0) as total_paid,
      (CASE WHEN b.vat_exempt = 1 THEN b.total ELSE ROUND(b.total * 1.12, 2) END)
  ...
  `).all() as {
    ...
    no_deposit: number; vat_exempt: number; fully_paid: number;
  ...
  const result = bookings.map(b => {
    ...
    if (b.status === 'completed') urgency = 'completed_unpaid';
