...
function DocView({ id }: { id: string }) {
  ...
  const balanceDue = subtotalExVAT - depositAmount; // balance is always on VAT-excl base per rate card
  ...
  return (
    <div className="min-h-screen bg-white p-6 md:p-10 max-w-[794px] mx-auto print-white" style={{ color: '#111', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '13px' }}>
          ...
          <tbody>
            {PAYMENT_ACCOUNTS.map((acc, i) => (
              <tr key={i}>
                ...
                <td style={{ padding: '3px 6px', color: '#333', fontFamily: 'monospace' }}>{acc.number}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function QuotationPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  return <Suspense><DocView id={id} /></Suspense>;
}
