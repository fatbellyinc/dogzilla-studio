...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (agency !== undefined) db.prepare('UPDATE projects SET agency = ? WHERE id = ?').run(agency || null, id);
  if (shoot_summary !== undefined) db.prepare('UPDATE projects SET shoot_summary = ? WHERE id = ?').run(shoot_summary || null, id);
  if (boards !== undefined) db.prepare('UPDATE projects SET boards = ? WHERE id = ?').run(boards || null, id);
  if (platforms !== undefined) db.prepare('UPDATE projects SET platforms = ? WHERE id = ?').run(platforms || null, id);
  if (talent_usage !== undefined) db.prepare('UPDATE projects SET talent_usage = ? WHERE id = ?').run(talent_usage || null, id);
  if (client_provides !== undefined) db.prepare('UPDATE projects SET client_provides = ? WHERE id = ?').run(client_provides || null, id);
